...
Curismo : A Smart Pill Box Assistant using Walabot and Amazon Alexa
Built on top of : Walabot's sensor targets project

"""

from __future__ import print_function, division
from datetime import datetime #for storing date
...
class TargetsPanel(tk.LabelFrame):
    ...
    def update(self, targets):
        global r_counter,l_counter,m_counter,rm_counter,lm_counter,mm_counter
        
        """ update the targets frame according to the given targets.
        """
        for i in range(self.master.numOfTargetsToDisplay):
            if i < len(targets):
                if (targets[i].zPosCm<20):
                    if(targets[i].yPosCm>10):
                        r_counter+=1
                        if(r_counter==1):
...
class CanvasPanel(tk.LabelFrame):
    ...
    def __init__(self, master):
        """ Init a black canvas.
        """
        tk.LabelFrame.__init__(self, master, text="Smart Pill Box: R / Phi")
...
def sensorTargets():
    ...
    """
    root = tk.Tk()
    root.title("Curismo - Smart Pill Box")
